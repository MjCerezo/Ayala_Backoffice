
package com.cifsdb.data;

import java.math.BigDecimal;
import java.util.Date;


/**
 *  CIFSDB.Tbloanfin
 *  09/26/2023 10:13:05
 * 
 */
public class Tbloanfin {

    private String txrefno;
    private String txbatch;
    private Date txdate;
    private String txcode;
    private Integer txoper;
    private String txmode;
    private Date txvaldt;
    private String slaidno;
    private String employeeno;
    private String lastname;
    private String firstname;
    private String middlename;
    private String accountno;
    private BigDecimal txamount;
    private BigDecimal txmisc;
    private BigDecimal txlpc;
    private BigDecimal txint;
    private BigDecimal txprin;
    private BigDecimal txamtbal;
    private String txstatus;
    private Date txstatusdate;
    private String createdby;
    private Date creationdate;
    private String approvedby;
    private Date approvaldate;
    private String particulars;
    private String reason;
    private Integer flag;
    private String txor;
    private String txchkno;
    private String txbrstn;
    private String txchecktype;
    private String pnno;
    private Date txordate;
    private String txchkbankname;
    private String txchkacctno;
    private String txchkacctname;
    private String txchkpayeename;
    private String txchkbankbr;
    private String txdebitacctno;
    private String txdebitaccttype;
    private Date txposteddate;
    private String remarks;
    private String clientname;
    private Date txchkdate;
    private String cifno;
    private String txsrce;
    private String legveh;
    private String grouprefno;
    private BigDecimal txchkamount;
    private String artype;
    private BigDecimal commrate;
    private BigDecimal commamt;
    private BigDecimal wtaxrate;
    private BigDecimal wtaxamt;
    private BigDecimal addtnlint;
    private String txdepositorybank;
    private String branchcode;
    private String newacctsts;
    private String oldacctsts;
    private BigDecimal addtnllpc;

    public String getTxrefno() {
        return txrefno;
    }

    public void setTxrefno(String txrefno) {
        this.txrefno = txrefno;
    }

    public String getTxbatch() {
        return txbatch;
    }

    public void setTxbatch(String txbatch) {
        this.txbatch = txbatch;
    }

    public Date getTxdate() {
        return txdate;
    }

    public void setTxdate(Date txdate) {
        this.txdate = txdate;
    }

    public String getTxcode() {
        return txcode;
    }

    public void setTxcode(String txcode) {
        this.txcode = txcode;
    }

    public Integer getTxoper() {
        return txoper;
    }

    public void setTxoper(Integer txoper) {
        this.txoper = txoper;
    }

    public String getTxmode() {
        return txmode;
    }

    public void setTxmode(String txmode) {
        this.txmode = txmode;
    }

    public Date getTxvaldt() {
        return txvaldt;
    }

    public void setTxvaldt(Date txvaldt) {
        this.txvaldt = txvaldt;
    }

    public String getSlaidno() {
        return slaidno;
    }

    public void setSlaidno(String slaidno) {
        this.slaidno = slaidno;
    }

    public String getEmployeeno() {
        return employeeno;
    }

    public void setEmployeeno(String employeeno) {
        this.employeeno = employeeno;
    }

    public String getLastname() {
        return lastname;
    }

    public void setLastname(String lastname) {
        this.lastname = lastname;
    }

    public String getFirstname() {
        return firstname;
    }

    public void setFirstname(String firstname) {
        this.firstname = firstname;
    }

    public String getMiddlename() {
        return middlename;
    }

    public void setMiddlename(String middlename) {
        this.middlename = middlename;
    }

    public String getAccountno() {
        return accountno;
    }

    public void setAccountno(String accountno) {
        this.accountno = accountno;
    }

    public BigDecimal getTxamount() {
        return txamount;
    }

    public void setTxamount(BigDecimal txamount) {
        this.txamount = txamount;
    }

    public BigDecimal getTxmisc() {
        return txmisc;
    }

    public void setTxmisc(BigDecimal txmisc) {
        this.txmisc = txmisc;
    }

    public BigDecimal getTxlpc() {
        return txlpc;
    }

    public void setTxlpc(BigDecimal txlpc) {
        this.txlpc = txlpc;
    }

    public BigDecimal getTxint() {
        return txint;
    }

    public void setTxint(BigDecimal txint) {
        this.txint = txint;
    }

    public BigDecimal getTxprin() {
        return txprin;
    }

    public void setTxprin(BigDecimal txprin) {
        this.txprin = txprin;
    }

    public BigDecimal getTxamtbal() {
        return txamtbal;
    }

    public void setTxamtbal(BigDecimal txamtbal) {
        this.txamtbal = txamtbal;
    }

    public String getTxstatus() {
        return txstatus;
    }

    public void setTxstatus(String txstatus) {
        this.txstatus = txstatus;
    }

    public Date getTxstatusdate() {
        return txstatusdate;
    }

    public void setTxstatusdate(Date txstatusdate) {
        this.txstatusdate = txstatusdate;
    }

    public String getCreatedby() {
        return createdby;
    }

    public void setCreatedby(String createdby) {
        this.createdby = createdby;
    }

    public Date getCreationdate() {
        return creationdate;
    }

    public void setCreationdate(Date creationdate) {
        this.creationdate = creationdate;
    }

    public String getApprovedby() {
        return approvedby;
    }

    public void setApprovedby(String approvedby) {
        this.approvedby = approvedby;
    }

    public Date getApprovaldate() {
        return approvaldate;
    }

    public void setApprovaldate(Date approvaldate) {
        this.approvaldate = approvaldate;
    }

    public String getParticulars() {
        return particulars;
    }

    public void setParticulars(String particulars) {
        this.particulars = particulars;
    }

    public String getReason() {
        return reason;
    }

    public void setReason(String reason) {
        this.reason = reason;
    }

    public Integer getFlag() {
        return flag;
    }

    public void setFlag(Integer flag) {
        this.flag = flag;
    }

    public String getTxor() {
        return txor;
    }

    public void setTxor(String txor) {
        this.txor = txor;
    }

    public String getTxchkno() {
        return txchkno;
    }

    public void setTxchkno(String txchkno) {
        this.txchkno = txchkno;
    }

    public String getTxbrstn() {
        return txbrstn;
    }

    public void setTxbrstn(String txbrstn) {
        this.txbrstn = txbrstn;
    }

    public String getTxchecktype() {
        return txchecktype;
    }

    public void setTxchecktype(String txchecktype) {
        this.txchecktype = txchecktype;
    }

    public String getPnno() {
        return pnno;
    }

    public void setPnno(String pnno) {
        this.pnno = pnno;
    }

    public Date getTxordate() {
        return txordate;
    }

    public void setTxordate(Date txordate) {
        this.txordate = txordate;
    }

    public String getTxchkbankname() {
        return txchkbankname;
    }

    public void setTxchkbankname(String txchkbankname) {
        this.txchkbankname = txchkbankname;
    }

    public String getTxchkacctno() {
        return txchkacctno;
    }

    public void setTxchkacctno(String txchkacctno) {
        this.txchkacctno = txchkacctno;
    }

    public String getTxchkacctname() {
        return txchkacctname;
    }

    public void setTxchkacctname(String txchkacctname) {
        this.txchkacctname = txchkacctname;
    }

    public String getTxchkpayeename() {
        return txchkpayeename;
    }

    public void setTxchkpayeename(String txchkpayeename) {
        this.txchkpayeename = txchkpayeename;
    }

    public String getTxchkbankbr() {
        return txchkbankbr;
    }

    public void setTxchkbankbr(String txchkbankbr) {
        this.txchkbankbr = txchkbankbr;
    }

    public String getTxdebitacctno() {
        return txdebitacctno;
    }

    public void setTxdebitacctno(String txdebitacctno) {
        this.txdebitacctno = txdebitacctno;
    }

    public String getTxdebitaccttype() {
        return txdebitaccttype;
    }

    public void setTxdebitaccttype(String txdebitaccttype) {
        this.txdebitaccttype = txdebitaccttype;
    }

    public Date getTxposteddate() {
        return txposteddate;
    }

    public void setTxposteddate(Date txposteddate) {
        this.txposteddate = txposteddate;
    }

    public String getRemarks() {
        return remarks;
    }

    public void setRemarks(String remarks) {
        this.remarks = remarks;
    }

    public String getClientname() {
        return clientname;
    }

    public void setClientname(String clientname) {
        this.clientname = clientname;
    }

    public Date getTxchkdate() {
        return txchkdate;
    }

    public void setTxchkdate(Date txchkdate) {
        this.txchkdate = txchkdate;
    }

    public String getCifno() {
        return cifno;
    }

    public void setCifno(String cifno) {
        this.cifno = cifno;
    }

    public String getTxsrce() {
        return txsrce;
    }

    public void setTxsrce(String txsrce) {
        this.txsrce = txsrce;
    }

    public String getLegveh() {
        return legveh;
    }

    public void setLegveh(String legveh) {
        this.legveh = legveh;
    }

    public String getGrouprefno() {
        return grouprefno;
    }

    public void setGrouprefno(String grouprefno) {
        this.grouprefno = grouprefno;
    }

    public BigDecimal getTxchkamount() {
        return txchkamount;
    }

    public void setTxchkamount(BigDecimal txchkamount) {
        this.txchkamount = txchkamount;
    }

    public String getArtype() {
        return artype;
    }

    public void setArtype(String artype) {
        this.artype = artype;
    }

    public BigDecimal getCommrate() {
        return commrate;
    }

    public void setCommrate(BigDecimal commrate) {
        this.commrate = commrate;
    }

    public BigDecimal getCommamt() {
        return commamt;
    }

    public void setCommamt(BigDecimal commamt) {
        this.commamt = commamt;
    }

    public BigDecimal getWtaxrate() {
        return wtaxrate;
    }

    public void setWtaxrate(BigDecimal wtaxrate) {
        this.wtaxrate = wtaxrate;
    }

    public BigDecimal getWtaxamt() {
        return wtaxamt;
    }

    public void setWtaxamt(BigDecimal wtaxamt) {
        this.wtaxamt = wtaxamt;
    }

    public BigDecimal getAddtnlint() {
        return addtnlint;
    }

    public void setAddtnlint(BigDecimal addtnlint) {
        this.addtnlint = addtnlint;
    }

    public String getTxdepositorybank() {
        return txdepositorybank;
    }

    public void setTxdepositorybank(String txdepositorybank) {
        this.txdepositorybank = txdepositorybank;
    }

    public String getBranchcode() {
        return branchcode;
    }

    public void setBranchcode(String branchcode) {
        this.branchcode = branchcode;
    }

    public String getNewacctsts() {
        return newacctsts;
    }

    public void setNewacctsts(String newacctsts) {
        this.newacctsts = newacctsts;
    }

    public String getOldacctsts() {
        return oldacctsts;
    }

    public void setOldacctsts(String oldacctsts) {
        this.oldacctsts = oldacctsts;
    }

    public BigDecimal getAddtnllpc() {
        return addtnllpc;
    }

    public void setAddtnllpc(BigDecimal addtnllpc) {
        this.addtnllpc = addtnllpc;
    }

}
