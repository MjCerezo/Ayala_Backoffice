
package com.cifsdb.data;

import java.util.Date;


/**
 *  CIFSDB.Tbcifindividual
 *  08/27/2024 14:22:04
 * 
 */
public class Tbcifindividual {

    private String cifno;
    private String lastname;
    private String firstname;
    private String middlename;
    private String shortname;
    private String suffix;
    private String previouslastname;
    private String preferredname;
    private String title;
    private Date dateofbirth;
    private Integer age;
    private String civilstatus;
    private String gender;
    private String placeofbirth;
    private String nationality;
    private Integer noofdependents;
    private String fulladdress1;
    private String streetno1;
    private String subdivision1;
    private String permanentaddstreetno;
    private String permanentaddstreetname;
    private String barangay1;
    private String province1;
    private String city1;
    private String country1;
    private String postalcode1;
    private String fulladdress2;
    private String streetno2;
    private String subdivision2;
    private String presentaddstreetno;
    private String presentaddstreetname;
    private String barangay2;
    private String province2;
    private String city2;
    private String country2;
    private String postalcode2;
    private Boolean issameaddress1;
    private Boolean issameaddress2;
    private String homeownership1;
    private String homeownership2;
    private String otherhomeownership1;
    private String otherhomeownership2;
    private Date occupiedsince1;
    private Date occupiedsince2;
    private Integer lengthofstaymo;
    private Integer lengthofstayyear;
    private String emailaddress;
    private String contacttype1;
    private String contactvalue1;
    private String contacttype2;
    private String contactvalue2;
    private String faxnumber;
    private String homecountrycode;
    private String homephonearea;
    private String homephoneno;
    private String mobilecountrycode;
    private String mobilephonearea;
    private String mobilenumber;
    private String officephonecountrycode;
    private String officephonearea;
    private String officephoneno;
    private String addresstype1;
    private String addresstype2;
    private String participationcode;
    private String relationtoborrower;
    private String tin;
    private String sss;
    private String statusloan;
    private String sector;
    private String sizeoffirm;
    private Integer spouseid;
    private String specificpurpose;
    private String idtype;
    private String idno;
    private Date issuedate;
    private Date expirydate;
    private String placeissued;
    private String countryofissue;
    private String countryofbirth;
    private String mailingaddress;
    private String billingaddress;
    private String mailingstreetno;
    private String mailinghouseno;
    private String mailingstreetname;
    private String mailingbarangay;
    private String mailingcity;
    private String mailingstate;
    private String mailingzipcode;
    private String mailingzipcodename;
    private Boolean mailingusesameaspresent;
    private Boolean mailingusesameaspermanent;
    private String postalcodename2;
    private String spousecifno;
    private Date spousedateofbirth;
    private String spousetitle;
    private String spousefirstname;
    private String spouselastname;
    private String spousemiddlename;
    private String spousesuffix;
    private Date fatherdateofbirth;
    private String fathercifno;
    private String fathertitle;
    private String fatherfirstname;
    private String fathermiddlename;
    private String fatherlastname;
    private String fathersuffix;
    private String mothercifno;
    private Date motherdateofbirth;
    private String mothertitle;
    private String motherfirstname;
    private String motherlastname;
    private String mothermiddlename;
    private String mothersuffix;
    private String presentaddzipcodename;
    private String permanentaddzipcodename;
    private String postalcodename1;
    private Boolean dualcitizen;
    private String othernationality;
    private Boolean resident;
    private String carsowned;
    private Boolean isqualifiedinvestor;
    private String coursepostgrad;
    private String schoolpostgrad;
    private String schoolcollege;
    private String coursecollege;
    private String sourceoffunds;
    private String othersourceoffunds;
    private String annualincome;
    private String asset;
    private String networth;
    private String cityofbirth;
    private String acrnumber;
    private String countrycodemobile;
    private String areacodemobile;
    private String countrycodephone;
    private String areacodephone;
    private String countrycodefax;
    private String areacodefax;
    private String usstateofbirth;
    private String usplaceofbirth;
    private String ussocialsecurityno;
    private String gsis;
    private String employmentstatus;
    private Boolean ismailingaddress;
    private String cifgroupcode;
    private String mothermaidenmname;
    private String beneficiarycifno;
    private Date beneficiarydateofbirth;
    private String beneficiarytitle;
    private String beneficiaryfirstname;
    private String beneficiarylastname;
    private String beneficiarymiddlename;
    private String beneficiarysuffix;
    private String indivoccupation;
    private String residenceclassification;
    private String appno;
    private String stateofbirth;
    private String religion;
    private String communitiestype1;
    private String communitiestype2;
    private String sexualpref;
    private String membershiptype;
    private Date datemembership;
    private String referredbycifno;
    private String referredbycifname;
    private String remarks;
    private String bankAccountNumber;
    private String bankName;

    public String getCifno() {
        return cifno;
    }

    public void setCifno(String cifno) {
        this.cifno = cifno;
    }

    public String getLastname() {
        return lastname;
    }

    public void setLastname(String lastname) {
        this.lastname = lastname;
    }

    public String getFirstname() {
        return firstname;
    }

    public void setFirstname(String firstname) {
        this.firstname = firstname;
    }

    public String getMiddlename() {
        return middlename;
    }

    public void setMiddlename(String middlename) {
        this.middlename = middlename;
    }

    public String getShortname() {
        return shortname;
    }

    public void setShortname(String shortname) {
        this.shortname = shortname;
    }

    public String getSuffix() {
        return suffix;
    }

    public void setSuffix(String suffix) {
        this.suffix = suffix;
    }

    public String getPreviouslastname() {
        return previouslastname;
    }

    public void setPreviouslastname(String previouslastname) {
        this.previouslastname = previouslastname;
    }

    public String getPreferredname() {
        return preferredname;
    }

    public void setPreferredname(String preferredname) {
        this.preferredname = preferredname;
    }

    public String getTitle() {
        return title;
    }

    public void setTitle(String title) {
        this.title = title;
    }

    public Date getDateofbirth() {
        return dateofbirth;
    }

    public void setDateofbirth(Date dateofbirth) {
        this.dateofbirth = dateofbirth;
    }

    public Integer getAge() {
        return age;
    }

    public void setAge(Integer age) {
        this.age = age;
    }

    public String getCivilstatus() {
        return civilstatus;
    }

    public void setCivilstatus(String civilstatus) {
        this.civilstatus = civilstatus;
    }

    public String getGender() {
        return gender;
    }

    public void setGender(String gender) {
        this.gender = gender;
    }

    public String getPlaceofbirth() {
        return placeofbirth;
    }

    public void setPlaceofbirth(String placeofbirth) {
        this.placeofbirth = placeofbirth;
    }

    public String getNationality() {
        return nationality;
    }

    public void setNationality(String nationality) {
        this.nationality = nationality;
    }

    public Integer getNoofdependents() {
        return noofdependents;
    }

    public void setNoofdependents(Integer noofdependents) {
        this.noofdependents = noofdependents;
    }

    public String getFulladdress1() {
        return fulladdress1;
    }

    public void setFulladdress1(String fulladdress1) {
        this.fulladdress1 = fulladdress1;
    }

    public String getStreetno1() {
        return streetno1;
    }

    public void setStreetno1(String streetno1) {
        this.streetno1 = streetno1;
    }

    public String getSubdivision1() {
        return subdivision1;
    }

    public void setSubdivision1(String subdivision1) {
        this.subdivision1 = subdivision1;
    }

    public String getPermanentaddstreetno() {
        return permanentaddstreetno;
    }

    public void setPermanentaddstreetno(String permanentaddstreetno) {
        this.permanentaddstreetno = permanentaddstreetno;
    }

    public String getPermanentaddstreetname() {
        return permanentaddstreetname;
    }

    public void setPermanentaddstreetname(String permanentaddstreetname) {
        this.permanentaddstreetname = permanentaddstreetname;
    }

    public String getBarangay1() {
        return barangay1;
    }

    public void setBarangay1(String barangay1) {
        this.barangay1 = barangay1;
    }

    public String getProvince1() {
        return province1;
    }

    public void setProvince1(String province1) {
        this.province1 = province1;
    }

    public String getCity1() {
        return city1;
    }

    public void setCity1(String city1) {
        this.city1 = city1;
    }

    public String getCountry1() {
        return country1;
    }

    public void setCountry1(String country1) {
        this.country1 = country1;
    }

    public String getPostalcode1() {
        return postalcode1;
    }

    public void setPostalcode1(String postalcode1) {
        this.postalcode1 = postalcode1;
    }

    public String getFulladdress2() {
        return fulladdress2;
    }

    public void setFulladdress2(String fulladdress2) {
        this.fulladdress2 = fulladdress2;
    }

    public String getStreetno2() {
        return streetno2;
    }

    public void setStreetno2(String streetno2) {
        this.streetno2 = streetno2;
    }

    public String getSubdivision2() {
        return subdivision2;
    }

    public void setSubdivision2(String subdivision2) {
        this.subdivision2 = subdivision2;
    }

    public String getPresentaddstreetno() {
        return presentaddstreetno;
    }

    public void setPresentaddstreetno(String presentaddstreetno) {
        this.presentaddstreetno = presentaddstreetno;
    }

    public String getPresentaddstreetname() {
        return presentaddstreetname;
    }

    public void setPresentaddstreetname(String presentaddstreetname) {
        this.presentaddstreetname = presentaddstreetname;
    }

    public String getBarangay2() {
        return barangay2;
    }

    public void setBarangay2(String barangay2) {
        this.barangay2 = barangay2;
    }

    public String getProvince2() {
        return province2;
    }

    public void setProvince2(String province2) {
        this.province2 = province2;
    }

    public String getCity2() {
        return city2;
    }

    public void setCity2(String city2) {
        this.city2 = city2;
    }

    public String getCountry2() {
        return country2;
    }

    public void setCountry2(String country2) {
        this.country2 = country2;
    }

    public String getPostalcode2() {
        return postalcode2;
    }

    public void setPostalcode2(String postalcode2) {
        this.postalcode2 = postalcode2;
    }

    public Boolean getIssameaddress1() {
        return issameaddress1;
    }

    public void setIssameaddress1(Boolean issameaddress1) {
        this.issameaddress1 = issameaddress1;
    }

    public Boolean getIssameaddress2() {
        return issameaddress2;
    }

    public void setIssameaddress2(Boolean issameaddress2) {
        this.issameaddress2 = issameaddress2;
    }

    public String getHomeownership1() {
        return homeownership1;
    }

    public void setHomeownership1(String homeownership1) {
        this.homeownership1 = homeownership1;
    }

    public String getHomeownership2() {
        return homeownership2;
    }

    public void setHomeownership2(String homeownership2) {
        this.homeownership2 = homeownership2;
    }

    public String getOtherhomeownership1() {
        return otherhomeownership1;
    }

    public void setOtherhomeownership1(String otherhomeownership1) {
        this.otherhomeownership1 = otherhomeownership1;
    }

    public String getOtherhomeownership2() {
        return otherhomeownership2;
    }

    public void setOtherhomeownership2(String otherhomeownership2) {
        this.otherhomeownership2 = otherhomeownership2;
    }

    public Date getOccupiedsince1() {
        return occupiedsince1;
    }

    public void setOccupiedsince1(Date occupiedsince1) {
        this.occupiedsince1 = occupiedsince1;
    }

    public Date getOccupiedsince2() {
        return occupiedsince2;
    }

    public void setOccupiedsince2(Date occupiedsince2) {
        this.occupiedsince2 = occupiedsince2;
    }

    public Integer getLengthofstaymo() {
        return lengthofstaymo;
    }

    public void setLengthofstaymo(Integer lengthofstaymo) {
        this.lengthofstaymo = lengthofstaymo;
    }

    public Integer getLengthofstayyear() {
        return lengthofstayyear;
    }

    public void setLengthofstayyear(Integer lengthofstayyear) {
        this.lengthofstayyear = lengthofstayyear;
    }

    public String getEmailaddress() {
        return emailaddress;
    }

    public void setEmailaddress(String emailaddress) {
        this.emailaddress = emailaddress;
    }

    public String getContacttype1() {
        return contacttype1;
    }

    public void setContacttype1(String contacttype1) {
        this.contacttype1 = contacttype1;
    }

    public String getContactvalue1() {
        return contactvalue1;
    }

    public void setContactvalue1(String contactvalue1) {
        this.contactvalue1 = contactvalue1;
    }

    public String getContacttype2() {
        return contacttype2;
    }

    public void setContacttype2(String contacttype2) {
        this.contacttype2 = contacttype2;
    }

    public String getContactvalue2() {
        return contactvalue2;
    }

    public void setContactvalue2(String contactvalue2) {
        this.contactvalue2 = contactvalue2;
    }

    public String getFaxnumber() {
        return faxnumber;
    }

    public void setFaxnumber(String faxnumber) {
        this.faxnumber = faxnumber;
    }

    public String getHomecountrycode() {
        return homecountrycode;
    }

    public void setHomecountrycode(String homecountrycode) {
        this.homecountrycode = homecountrycode;
    }

    public String getHomephonearea() {
        return homephonearea;
    }

    public void setHomephonearea(String homephonearea) {
        this.homephonearea = homephonearea;
    }

    public String getHomephoneno() {
        return homephoneno;
    }

    public void setHomephoneno(String homephoneno) {
        this.homephoneno = homephoneno;
    }

    public String getMobilecountrycode() {
        return mobilecountrycode;
    }

    public void setMobilecountrycode(String mobilecountrycode) {
        this.mobilecountrycode = mobilecountrycode;
    }

    public String getMobilephonearea() {
        return mobilephonearea;
    }

    public void setMobilephonearea(String mobilephonearea) {
        this.mobilephonearea = mobilephonearea;
    }

    public String getMobilenumber() {
        return mobilenumber;
    }

    public void setMobilenumber(String mobilenumber) {
        this.mobilenumber = mobilenumber;
    }

    public String getOfficephonecountrycode() {
        return officephonecountrycode;
    }

    public void setOfficephonecountrycode(String officephonecountrycode) {
        this.officephonecountrycode = officephonecountrycode;
    }

    public String getOfficephonearea() {
        return officephonearea;
    }

    public void setOfficephonearea(String officephonearea) {
        this.officephonearea = officephonearea;
    }

    public String getOfficephoneno() {
        return officephoneno;
    }

    public void setOfficephoneno(String officephoneno) {
        this.officephoneno = officephoneno;
    }

    public String getAddresstype1() {
        return addresstype1;
    }

    public void setAddresstype1(String addresstype1) {
        this.addresstype1 = addresstype1;
    }

    public String getAddresstype2() {
        return addresstype2;
    }

    public void setAddresstype2(String addresstype2) {
        this.addresstype2 = addresstype2;
    }

    public String getParticipationcode() {
        return participationcode;
    }

    public void setParticipationcode(String participationcode) {
        this.participationcode = participationcode;
    }

    public String getRelationtoborrower() {
        return relationtoborrower;
    }

    public void setRelationtoborrower(String relationtoborrower) {
        this.relationtoborrower = relationtoborrower;
    }

    public String getTin() {
        return tin;
    }

    public void setTin(String tin) {
        this.tin = tin;
    }

    public String getSss() {
        return sss;
    }

    public void setSss(String sss) {
        this.sss = sss;
    }

    public String getStatusloan() {
        return statusloan;
    }

    public void setStatusloan(String statusloan) {
        this.statusloan = statusloan;
    }

    public String getSector() {
        return sector;
    }

    public void setSector(String sector) {
        this.sector = sector;
    }

    public String getSizeoffirm() {
        return sizeoffirm;
    }

    public void setSizeoffirm(String sizeoffirm) {
        this.sizeoffirm = sizeoffirm;
    }

    public Integer getSpouseid() {
        return spouseid;
    }

    public void setSpouseid(Integer spouseid) {
        this.spouseid = spouseid;
    }

    public String getSpecificpurpose() {
        return specificpurpose;
    }

    public void setSpecificpurpose(String specificpurpose) {
        this.specificpurpose = specificpurpose;
    }

    public String getIdtype() {
        return idtype;
    }

    public void setIdtype(String idtype) {
        this.idtype = idtype;
    }

    public String getIdno() {
        return idno;
    }

    public void setIdno(String idno) {
        this.idno = idno;
    }

    public Date getIssuedate() {
        return issuedate;
    }

    public void setIssuedate(Date issuedate) {
        this.issuedate = issuedate;
    }

    public Date getExpirydate() {
        return expirydate;
    }

    public void setExpirydate(Date expirydate) {
        this.expirydate = expirydate;
    }

    public String getPlaceissued() {
        return placeissued;
    }

    public void setPlaceissued(String placeissued) {
        this.placeissued = placeissued;
    }

    public String getCountryofissue() {
        return countryofissue;
    }

    public void setCountryofissue(String countryofissue) {
        this.countryofissue = countryofissue;
    }

    public String getCountryofbirth() {
        return countryofbirth;
    }

    public void setCountryofbirth(String countryofbirth) {
        this.countryofbirth = countryofbirth;
    }

    public String getMailingaddress() {
        return mailingaddress;
    }

    public void setMailingaddress(String mailingaddress) {
        this.mailingaddress = mailingaddress;
    }

    public String getBillingaddress() {
        return billingaddress;
    }

    public void setBillingaddress(String billingaddress) {
        this.billingaddress = billingaddress;
    }

    public String getMailingstreetno() {
        return mailingstreetno;
    }

    public void setMailingstreetno(String mailingstreetno) {
        this.mailingstreetno = mailingstreetno;
    }

    public String getMailinghouseno() {
        return mailinghouseno;
    }

    public void setMailinghouseno(String mailinghouseno) {
        this.mailinghouseno = mailinghouseno;
    }

    public String getMailingstreetname() {
        return mailingstreetname;
    }

    public void setMailingstreetname(String mailingstreetname) {
        this.mailingstreetname = mailingstreetname;
    }

    public String getMailingbarangay() {
        return mailingbarangay;
    }

    public void setMailingbarangay(String mailingbarangay) {
        this.mailingbarangay = mailingbarangay;
    }

    public String getMailingcity() {
        return mailingcity;
    }

    public void setMailingcity(String mailingcity) {
        this.mailingcity = mailingcity;
    }

    public String getMailingstate() {
        return mailingstate;
    }

    public void setMailingstate(String mailingstate) {
        this.mailingstate = mailingstate;
    }

    public String getMailingzipcode() {
        return mailingzipcode;
    }

    public void setMailingzipcode(String mailingzipcode) {
        this.mailingzipcode = mailingzipcode;
    }

    public String getMailingzipcodename() {
        return mailingzipcodename;
    }

    public void setMailingzipcodename(String mailingzipcodename) {
        this.mailingzipcodename = mailingzipcodename;
    }

    public Boolean getMailingusesameaspresent() {
        return mailingusesameaspresent;
    }

    public void setMailingusesameaspresent(Boolean mailingusesameaspresent) {
        this.mailingusesameaspresent = mailingusesameaspresent;
    }

    public Boolean getMailingusesameaspermanent() {
        return mailingusesameaspermanent;
    }

    public void setMailingusesameaspermanent(Boolean mailingusesameaspermanent) {
        this.mailingusesameaspermanent = mailingusesameaspermanent;
    }

    public String getPostalcodename2() {
        return postalcodename2;
    }

    public void setPostalcodename2(String postalcodename2) {
        this.postalcodename2 = postalcodename2;
    }

    public String getSpousecifno() {
        return spousecifno;
    }

    public void setSpousecifno(String spousecifno) {
        this.spousecifno = spousecifno;
    }

    public Date getSpousedateofbirth() {
        return spousedateofbirth;
    }

    public void setSpousedateofbirth(Date spousedateofbirth) {
        this.spousedateofbirth = spousedateofbirth;
    }

    public String getSpousetitle() {
        return spousetitle;
    }

    public void setSpousetitle(String spousetitle) {
        this.spousetitle = spousetitle;
    }

    public String getSpousefirstname() {
        return spousefirstname;
    }

    public void setSpousefirstname(String spousefirstname) {
        this.spousefirstname = spousefirstname;
    }

    public String getSpouselastname() {
        return spouselastname;
    }

    public void setSpouselastname(String spouselastname) {
        this.spouselastname = spouselastname;
    }

    public String getSpousemiddlename() {
        return spousemiddlename;
    }

    public void setSpousemiddlename(String spousemiddlename) {
        this.spousemiddlename = spousemiddlename;
    }

    public String getSpousesuffix() {
        return spousesuffix;
    }

    public void setSpousesuffix(String spousesuffix) {
        this.spousesuffix = spousesuffix;
    }

    public Date getFatherdateofbirth() {
        return fatherdateofbirth;
    }

    public void setFatherdateofbirth(Date fatherdateofbirth) {
        this.fatherdateofbirth = fatherdateofbirth;
    }

    public String getFathercifno() {
        return fathercifno;
    }

    public void setFathercifno(String fathercifno) {
        this.fathercifno = fathercifno;
    }

    public String getFathertitle() {
        return fathertitle;
    }

    public void setFathertitle(String fathertitle) {
        this.fathertitle = fathertitle;
    }

    public String getFatherfirstname() {
        return fatherfirstname;
    }

    public void setFatherfirstname(String fatherfirstname) {
        this.fatherfirstname = fatherfirstname;
    }

    public String getFathermiddlename() {
        return fathermiddlename;
    }

    public void setFathermiddlename(String fathermiddlename) {
        this.fathermiddlename = fathermiddlename;
    }

    public String getFatherlastname() {
        return fatherlastname;
    }

    public void setFatherlastname(String fatherlastname) {
        this.fatherlastname = fatherlastname;
    }

    public String getFathersuffix() {
        return fathersuffix;
    }

    public void setFathersuffix(String fathersuffix) {
        this.fathersuffix = fathersuffix;
    }

    public String getMothercifno() {
        return mothercifno;
    }

    public void setMothercifno(String mothercifno) {
        this.mothercifno = mothercifno;
    }

    public Date getMotherdateofbirth() {
        return motherdateofbirth;
    }

    public void setMotherdateofbirth(Date motherdateofbirth) {
        this.motherdateofbirth = motherdateofbirth;
    }

    public String getMothertitle() {
        return mothertitle;
    }

    public void setMothertitle(String mothertitle) {
        this.mothertitle = mothertitle;
    }

    public String getMotherfirstname() {
        return motherfirstname;
    }

    public void setMotherfirstname(String motherfirstname) {
        this.motherfirstname = motherfirstname;
    }

    public String getMotherlastname() {
        return motherlastname;
    }

    public void setMotherlastname(String motherlastname) {
        this.motherlastname = motherlastname;
    }

    public String getMothermiddlename() {
        return mothermiddlename;
    }

    public void setMothermiddlename(String mothermiddlename) {
        this.mothermiddlename = mothermiddlename;
    }

    public String getMothersuffix() {
        return mothersuffix;
    }

    public void setMothersuffix(String mothersuffix) {
        this.mothersuffix = mothersuffix;
    }

    public String getPresentaddzipcodename() {
        return presentaddzipcodename;
    }

    public void setPresentaddzipcodename(String presentaddzipcodename) {
        this.presentaddzipcodename = presentaddzipcodename;
    }

    public String getPermanentaddzipcodename() {
        return permanentaddzipcodename;
    }

    public void setPermanentaddzipcodename(String permanentaddzipcodename) {
        this.permanentaddzipcodename = permanentaddzipcodename;
    }

    public String getPostalcodename1() {
        return postalcodename1;
    }

    public void setPostalcodename1(String postalcodename1) {
        this.postalcodename1 = postalcodename1;
    }

    public Boolean getDualcitizen() {
        return dualcitizen;
    }

    public void setDualcitizen(Boolean dualcitizen) {
        this.dualcitizen = dualcitizen;
    }

    public String getOthernationality() {
        return othernationality;
    }

    public void setOthernationality(String othernationality) {
        this.othernationality = othernationality;
    }

    public Boolean getResident() {
        return resident;
    }

    public void setResident(Boolean resident) {
        this.resident = resident;
    }

    public String getCarsowned() {
        return carsowned;
    }

    public void setCarsowned(String carsowned) {
        this.carsowned = carsowned;
    }

    public Boolean getIsqualifiedinvestor() {
        return isqualifiedinvestor;
    }

    public void setIsqualifiedinvestor(Boolean isqualifiedinvestor) {
        this.isqualifiedinvestor = isqualifiedinvestor;
    }

    public String getCoursepostgrad() {
        return coursepostgrad;
    }

    public void setCoursepostgrad(String coursepostgrad) {
        this.coursepostgrad = coursepostgrad;
    }

    public String getSchoolpostgrad() {
        return schoolpostgrad;
    }

    public void setSchoolpostgrad(String schoolpostgrad) {
        this.schoolpostgrad = schoolpostgrad;
    }

    public String getSchoolcollege() {
        return schoolcollege;
    }

    public void setSchoolcollege(String schoolcollege) {
        this.schoolcollege = schoolcollege;
    }

    public String getCoursecollege() {
        return coursecollege;
    }

    public void setCoursecollege(String coursecollege) {
        this.coursecollege = coursecollege;
    }

    public String getSourceoffunds() {
        return sourceoffunds;
    }

    public void setSourceoffunds(String sourceoffunds) {
        this.sourceoffunds = sourceoffunds;
    }

    public String getOthersourceoffunds() {
        return othersourceoffunds;
    }

    public void setOthersourceoffunds(String othersourceoffunds) {
        this.othersourceoffunds = othersourceoffunds;
    }

    public String getAnnualincome() {
        return annualincome;
    }

    public void setAnnualincome(String annualincome) {
        this.annualincome = annualincome;
    }

    public String getAsset() {
        return asset;
    }

    public void setAsset(String asset) {
        this.asset = asset;
    }

    public String getNetworth() {
        return networth;
    }

    public void setNetworth(String networth) {
        this.networth = networth;
    }

    public String getCityofbirth() {
        return cityofbirth;
    }

    public void setCityofbirth(String cityofbirth) {
        this.cityofbirth = cityofbirth;
    }

    public String getAcrnumber() {
        return acrnumber;
    }

    public void setAcrnumber(String acrnumber) {
        this.acrnumber = acrnumber;
    }

    public String getCountrycodemobile() {
        return countrycodemobile;
    }

    public void setCountrycodemobile(String countrycodemobile) {
        this.countrycodemobile = countrycodemobile;
    }

    public String getAreacodemobile() {
        return areacodemobile;
    }

    public void setAreacodemobile(String areacodemobile) {
        this.areacodemobile = areacodemobile;
    }

    public String getCountrycodephone() {
        return countrycodephone;
    }

    public void setCountrycodephone(String countrycodephone) {
        this.countrycodephone = countrycodephone;
    }

    public String getAreacodephone() {
        return areacodephone;
    }

    public void setAreacodephone(String areacodephone) {
        this.areacodephone = areacodephone;
    }

    public String getCountrycodefax() {
        return countrycodefax;
    }

    public void setCountrycodefax(String countrycodefax) {
        this.countrycodefax = countrycodefax;
    }

    public String getAreacodefax() {
        return areacodefax;
    }

    public void setAreacodefax(String areacodefax) {
        this.areacodefax = areacodefax;
    }

    public String getUsstateofbirth() {
        return usstateofbirth;
    }

    public void setUsstateofbirth(String usstateofbirth) {
        this.usstateofbirth = usstateofbirth;
    }

    public String getUsplaceofbirth() {
        return usplaceofbirth;
    }

    public void setUsplaceofbirth(String usplaceofbirth) {
        this.usplaceofbirth = usplaceofbirth;
    }

    public String getUssocialsecurityno() {
        return ussocialsecurityno;
    }

    public void setUssocialsecurityno(String ussocialsecurityno) {
        this.ussocialsecurityno = ussocialsecurityno;
    }

    public String getGsis() {
        return gsis;
    }

    public void setGsis(String gsis) {
        this.gsis = gsis;
    }

    public String getEmploymentstatus() {
        return employmentstatus;
    }

    public void setEmploymentstatus(String employmentstatus) {
        this.employmentstatus = employmentstatus;
    }

    public Boolean getIsmailingaddress() {
        return ismailingaddress;
    }

    public void setIsmailingaddress(Boolean ismailingaddress) {
        this.ismailingaddress = ismailingaddress;
    }

    public String getCifgroupcode() {
        return cifgroupcode;
    }

    public void setCifgroupcode(String cifgroupcode) {
        this.cifgroupcode = cifgroupcode;
    }

    public String getMothermaidenmname() {
        return mothermaidenmname;
    }

    public void setMothermaidenmname(String mothermaidenmname) {
        this.mothermaidenmname = mothermaidenmname;
    }

    public String getBeneficiarycifno() {
        return beneficiarycifno;
    }

    public void setBeneficiarycifno(String beneficiarycifno) {
        this.beneficiarycifno = beneficiarycifno;
    }

    public Date getBeneficiarydateofbirth() {
        return beneficiarydateofbirth;
    }

    public void setBeneficiarydateofbirth(Date beneficiarydateofbirth) {
        this.beneficiarydateofbirth = beneficiarydateofbirth;
    }

    public String getBeneficiarytitle() {
        return beneficiarytitle;
    }

    public void setBeneficiarytitle(String beneficiarytitle) {
        this.beneficiarytitle = beneficiarytitle;
    }

    public String getBeneficiaryfirstname() {
        return beneficiaryfirstname;
    }

    public void setBeneficiaryfirstname(String beneficiaryfirstname) {
        this.beneficiaryfirstname = beneficiaryfirstname;
    }

    public String getBeneficiarylastname() {
        return beneficiarylastname;
    }

    public void setBeneficiarylastname(String beneficiarylastname) {
        this.beneficiarylastname = beneficiarylastname;
    }

    public String getBeneficiarymiddlename() {
        return beneficiarymiddlename;
    }

    public void setBeneficiarymiddlename(String beneficiarymiddlename) {
        this.beneficiarymiddlename = beneficiarymiddlename;
    }

    public String getBeneficiarysuffix() {
        return beneficiarysuffix;
    }

    public void setBeneficiarysuffix(String beneficiarysuffix) {
        this.beneficiarysuffix = beneficiarysuffix;
    }

    public String getIndivoccupation() {
        return indivoccupation;
    }

    public void setIndivoccupation(String indivoccupation) {
        this.indivoccupation = indivoccupation;
    }

    public String getResidenceclassification() {
        return residenceclassification;
    }

    public void setResidenceclassification(String residenceclassification) {
        this.residenceclassification = residenceclassification;
    }

    public String getAppno() {
        return appno;
    }

    public void setAppno(String appno) {
        this.appno = appno;
    }

    public String getStateofbirth() {
        return stateofbirth;
    }

    public void setStateofbirth(String stateofbirth) {
        this.stateofbirth = stateofbirth;
    }

    public String getReligion() {
        return religion;
    }

    public void setReligion(String religion) {
        this.religion = religion;
    }

    public String getCommunitiestype1() {
        return communitiestype1;
    }

    public void setCommunitiestype1(String communitiestype1) {
        this.communitiestype1 = communitiestype1;
    }

    public String getCommunitiestype2() {
        return communitiestype2;
    }

    public void setCommunitiestype2(String communitiestype2) {
        this.communitiestype2 = communitiestype2;
    }

    public String getSexualpref() {
        return sexualpref;
    }

    public void setSexualpref(String sexualpref) {
        this.sexualpref = sexualpref;
    }

    public String getMembershiptype() {
        return membershiptype;
    }

    public void setMembershiptype(String membershiptype) {
        this.membershiptype = membershiptype;
    }

    public Date getDatemembership() {
        return datemembership;
    }

    public void setDatemembership(Date datemembership) {
        this.datemembership = datemembership;
    }

    public String getReferredbycifno() {
        return referredbycifno;
    }

    public void setReferredbycifno(String referredbycifno) {
        this.referredbycifno = referredbycifno;
    }

    public String getReferredbycifname() {
        return referredbycifname;
    }

    public void setReferredbycifname(String referredbycifname) {
        this.referredbycifname = referredbycifname;
    }

    public String getRemarks() {
        return remarks;
    }

    public void setRemarks(String remarks) {
        this.remarks = remarks;
    }

    public String getBankAccountNumber() {
        return bankAccountNumber;
    }

    public void setBankAccountNumber(String bankAccountNumber) {
        this.bankAccountNumber = bankAccountNumber;
    }

    public String getBankName() {
        return bankName;
    }

    public void setBankName(String bankName) {
        this.bankName = bankName;
    }

}
