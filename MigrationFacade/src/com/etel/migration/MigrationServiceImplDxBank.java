 ...
 *
 */
public class MigrationServiceImplDxBank implements MigrationService{
...
//					sheetCorp = sheet;
			...
			if (sheetCIF != null) {
				for (int i = 1; i < sheetCIF.getPhysicalNumberOfRows(); i++) {
					Row row = sheetCIF.getRow(i);
...
//					}
					...
					cifmain.setOldcifno(formatter.formatCellValue(row.getCell(1)));
					...
					if (cifmain.getCustomertype().equals("1")) {
						...
						cifmain.setFullname(indiv.getLastname() + ", " + indiv.getFirstname()
...
//							DateTimeUtil.DATE_FORMAT_MM_DD_YYYY));
//					employment.setIspresent(row.getCell(6).getBooleanCellValue());
					employment.setGrossincome((int) row.getCell(7).getNumericCellValue());
			...
			if (sheetBusiness != null) {
				for (int iBusiness = 1; iBusiness < sheetBusiness.getPhysicalNumberOfRows(); iBusiness++) {
					...
					business.setCifno(formatter.formatCellValue(row.getCell(0)));
					...
					business.setCountrycodefax(formatter.formatCellValue(row.getCell(4)));
					business.setEmailaddress(formatter.formatCellValue(row.getCell(5)));
					business.setGrossincome(
							BigDecimal.valueOf(row.getCell(6).getNumericCellValue()).setScale(2, RoundingMode.HALF_UP));
					business.setBusinessclass(formatter.formatCellValue(row.getCell(7)));
		...
		try {
			...
			if (sheetDeposit != null) {
				...
				for (int i = 1; i < sheetDeposit.getPhysicalNumberOfRows(); i++) {
					...
					param.put("cifno", formatter.formatCellValue(row.getCell(1)));
					param.put("oldcifno", formatter.formatCellValue(row.getCell(2)));
					// MAIN CIF ACCOUNT
					...
					if (cifList == null || cifList.size() == 0) {
						...
						log.setUniquekey(formatter.formatCellValue(row.getCell(1)));
						dbsrvc.save(log);
						continue;
					}
					param.put("acctno", formatter.formatCellValue(row.getCell(2)));
...
//							dep.getEmployeeNo());
					...
					if (acctResult.getResult() == 1) {
						if (formatter.formatCellValue(row.getCell(30)).length() > 0) {
							...
							pb.setNewpssbksn(formatter.formatCellValue(row.getCell(30)));
							pb.setOldpassbksn("");
...
//					dep.setLasttxdate(DateTimeUtil.convertToDate(formatter.formatCellValue(row.getCell(19)),
//							DateTimeUtil.DATE_FORMAT_MM_DD_YYYY));
//					dbsrvc.saveOrUpdate(dep);
				}
			}
		...
		try {
			...
			if (sheetLoans != null) {
				for (int i = 1; i < sheetLoans.getPhysicalNumberOfRows(); i++) {
					...
					param.put("oldcifno", formatter.formatCellValue(row.getCell(1)));
					// MAIN CIF ACCOUNT
					Tbcifmain cifmain = (Tbcifmain) dbsrvcCIF.execStoredProc(
							"SELECT * FROM Tbcifmain WHERE oldcifno=:oldcifno", param, Tbcifmain.class, 0, null);
					if (cifmain == null) {
					...
					if (dbService.save(lstapp)) {
						...
						form.setAmountfinanced(BigDecimal.valueOf(row.getCell(19).getNumericCellValue()).setScale(2,
								RoundingMode.HALF_UP));
						form.setLoanpurpose(formatter.formatCellValue(row.getCell(28)));
						...
						form.setDuedaterule(formatter.formatCellValue(row.getCell(31)));
						...
						Tbloans loan = (Tbloans) dbService.executeUniqueHQLQuery("FROM Tbloans WHERE accountno=:acctno",
								param);
						loan.setLpcrate(BigDecimal.valueOf(row.getCell(24).getNumericCellValue()).setScale(4,
								RoundingMode.HALF_UP));
						loan.setAcctsts((int) row.getCell(25).getNumericCellValue());
						loan.setAcctstsDate(DateTimeUtil.convertToDate(formatter.formatCellValue(row.getCell(26)),
								DateTimeUtil.DATE_FORMAT_MM_DD_YYYY));
						loan.setLsttxdt(DateTimeUtil.convertToDate(formatter.formatCellValue(row.getCell(27)),
								DateTimeUtil.DATE_FORMAT_MM_DD_YYYY));
...
//						loanfin.setTxmisc(
//								BigDecimal.valueOf(row.getCell(22).getNumericCellValue()).setScale(2, RoundingMode.HALF_UP));
//						loanfin.setTxlpc(
//								BigDecimal.valueOf(row.getCell(23).getNumericCellValue()).setScale(2, RoundingMode.HALF_UP));
						loanfin.setTxamount(loanfin.getTxprin().add(loanfin.getTxint()));
						...
						if (loanfin.getTxamount().compareTo(BigDecimal.ZERO) == 1) {
							...
							loanfin.setTxdate(branch.getCurrentbusinessdate());
							loanfin.setTxmode("4");
							loanfin.setTxoper(2);
							loanfin.setTxor("");
							...
							loanfin.setTxvaldt(DateTimeUtil.convertToDate(formatter.formatCellValue(row.getCell(27)),
									DateTimeUtil.DATE_FORMAT_MM_DD_YYYY));
