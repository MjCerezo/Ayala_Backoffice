...
import com.etel.loancalc.LoanCalculatorServiceImplNew;
import com.etel.loancalc.LoanCalculatorServiceNew;
...
public class MigrationServiceImpl implements MigrationService {
	...
	public FileUploadResponse uploadXLSXFile(MultipartFile file) throws IOException {
		...
		String uploadDir = RuntimeAccess.getInstance().getSession().getServletContext()
				.getRealPath("resources/data/migration");
		...
		try {
			...
			if (!ext.equals(".xlsx") && !ext.equals(".xls")) {
				ret.setError("Please select an excel file.");
				return ret;
			}
	...
	public List<MigrationResultForm> uploadCIF(String filepath) {
		...
		try {
			...
			Sheet sheetIndividual = null;
			Sheet sheetEmployment = null;
			Sheet sheetBusiness = null;
//			Sheet sheetCorp = null;
			...
			for (Sheet sheet : workbook) {
				if (sheet.getSheetName().equals("CIF - INDIVIDUAL")) {
					sheetIndividual = sheet;
				}
				if (sheet.getSheetName().equals("CIF - EMPLOYMENT")) {
					sheetEmployment = sheet;
				}
				if (sheet.getSheetName().equals("CIF - BUSINESS")) {
					sheetBusiness = sheet;
				}
				if (sheet.getSheetName().equals("CIF - CORPORATE")) {
//					sheetCorp = sheet;
				}
			}

			if (sheetIndividual != null) {
				Tblogs log = new Tblogs();
				String batchcode = (String) dbsrvc.executeUniqueSQLQuery(
				...
				log.setDescription(
						"BATCH CIF UPLOAD - " + filepath.substring(filepath.lastIndexOf("\\") + 1, filepath.length()));
				log.setErrordescription("");
				log.setEventdate(new Date());
				log.setEventname("BATCH CIF INDIVIDUAL");
				log.setModulename("BATCH UPLOAD");
				log.setNextdate(branch.getNextbusinessdate());
				log.setUniquekey(batchcode);
				dbsrvc.save(log);
				for (int i = 1; i < sheetIndividual.getPhysicalNumberOfRows(); i++) {
					Row row = sheetIndividual.getRow(i);
					System.out.println(i + " >> " + formatter.formatCellValue(row.getCell(1)));
					if (formatter.formatCellValue(row.getCell(0)) == null) {
						log.setErrordescription("Branch column is empty.");
						log.setUniquekey("Row number : " + i);
						dbsrvc.save(log);
						continue;
					}
					param.put("branchcode", formatter.formatCellValue(row.getCell(0)));
					branch = (Tbbranch) dbsrvc.executeUniqueHQLQuery("FROM Tbbranch WHERE branchcode=:branchcode",
							param);
					if (formatter.formatCellValue(row.getCell(0)) == null) {
						log.setErrordescription("Branch code does not exist in database.");
						log.setUniquekey("Row number : " + i);
						dbsrvc.save(log);
						continue;
					}
					param.put("oldcifno", formatter.formatCellValue(row.getCell(1)).trim());
					Tbcifmain cifmain = new Tbcifmain();
					cifmain = (Tbcifmain) dbsrvcCIF.executeUniqueHQLQuery("FROM Tbcifmain WHERE oldcifno=:oldcifno",
							param);
					if (cifmain != null) {
						log.setErrordescription("Duplicate Old CIF Number.");
						log.setUniquekey("Row number : " + i);
						dbsrvc.save(log);
						continue;
					}
					cifmain = new Tbcifmain();
					cifmain.setOriginatingbranch(formatter.formatCellValue(row.getCell(0)));
					cifmain.setOldcifno(formatter.formatCellValue(row.getCell(1)).trim());
					cifmain.setCustomertype(formatter.formatCellValue(row.getCell(2)));
					cifmain.setEncodedby(user.getUsername());
					cifmain.setIsencoding(true);
					cifmain.setEncodeddate(branch.getCurrentbusinessdate());
					cifmain.setAssignedto(user.getUsername());
					cifmain.setDateupdated(new Date());
					cifmain.setCifstatus("3");
					cifmain.setCifstatusdate(new Date());
					cifmain.setCifapprovedby(user.getUsername());
					cifmain.setCifapproveddate(new Date());
					cifmain.setCiftype("2");
					cifmain.setBorrowerfundertype("2");
					cifmain.setApprovalcode("4");// ced for clarification
					cifmain.setDateofbirth(DateTimeUtil.convertToDate(formatter.formatCellValue(row.getCell(13)),
							DateTimeUtil.DATE_FORMAT_MM_DD_YYYY));
					if (cifmain.getCustomertype().equals("1")) {
						// INDIVIDUAL
						Tbcifindividual indiv = new Tbcifindividual();
						indiv.setTitle(formatter.formatCellValue(row.getCell(3)));
						indiv.setFirstname(formatter.formatCellValue(row.getCell(4)).trim());
						indiv.setMiddlename(formatter.formatCellValue(row.getCell(5)).trim());
						indiv.setLastname(formatter.formatCellValue(row.getCell(6)).trim());
						indiv.setSuffix(formatter.formatCellValue(row.getCell(7)));
						indiv.setShortname(formatter.formatCellValue(row.getCell(8)));
						indiv.setPreviouslastname(formatter.formatCellValue(row.getCell(9)));
						indiv.setGender(formatter.formatCellValue(row.getCell(10)));
						indiv.setSexualpref(formatter.formatCellValue(row.getCell(11)));
						indiv.setCivilstatus(formatter.formatCellValue(row.getCell(12)));
						indiv.setDateofbirth(DateTimeUtil.convertToDate(formatter.formatCellValue(row.getCell(13)),
								DateTimeUtil.DATE_FORMAT_MM_DD_YYYY));
						indiv.setCountryofbirth(formatter.formatCellValue(row.getCell(14)));
						indiv.setPlaceofbirth(formatter.formatCellValue(row.getCell(15)));
						indiv.setStateofbirth(formatter.formatCellValue(row.getCell(15)));
						indiv.setCityofbirth(formatter.formatCellValue(row.getCell(16)));
						indiv.setNationality(formatter.formatCellValue(row.getCell(17)));
						indiv.setDualcitizen(row.getCell(18).getBooleanCellValue());
						indiv.setResident(row.getCell(19).getBooleanCellValue());
						indiv.setTin(formatter.formatCellValue(row.getCell(20)));
						indiv.setSss(formatter.formatCellValue(row.getCell(21)));
						indiv.setGsis(formatter.formatCellValue(row.getCell(22)));
						indiv.setReligion(formatter.formatCellValue(row.getCell(23)));
						indiv.setMobilenumber(formatter.formatCellValue(row.getCell(24)));
						indiv.setHomephoneno(formatter.formatCellValue(row.getCell(25)));
						indiv.setFaxnumber(formatter.formatCellValue(row.getCell(26)));
						indiv.setEmailaddress(formatter.formatCellValue(row.getCell(27)));
						indiv.setContacttype1(formatter.formatCellValue(row.getCell(28)));
						indiv.setContactvalue1(formatter.formatCellValue(row.getCell(29)));
						indiv.setContacttype2(formatter.formatCellValue(row.getCell(30)));
						indiv.setContactvalue2(formatter.formatCellValue(row.getCell(31)));
						indiv.setStreetno1(formatter.formatCellValue(row.getCell(32)));
						indiv.setSubdivision1(formatter.formatCellValue(row.getCell(33)));
						indiv.setCountry1(formatter.formatCellValue(row.getCell(34)));
						indiv.setProvince1(formatter.formatCellValue(row.getCell(35)));
						indiv.setCity1(formatter.formatCellValue(row.getCell(36)));
						indiv.setBarangay1(formatter.formatCellValue(row.getCell(37)));
						indiv.setPostalcode1(formatter.formatCellValue(row.getCell(38)));
						indiv.setHomeownership1(formatter.formatCellValue(row.getCell(39)));
						indiv.setOccupiedsince1(DateTimeUtil.convertToDate(formatter.formatCellValue(row.getCell(40)),
								DateTimeUtil.DATE_FORMAT_MM_DD_YYYY));
//						indiv.setAddresstype1(formatter.formatCellValue(row.getCell(41)));
						indiv.setCommunitiestype1(formatter.formatCellValue(row.getCell(41)));
						...
						indiv.setStreetno2(formatter.formatCellValue(row.getCell(42)));
						indiv.setSubdivision2(formatter.formatCellValue(row.getCell(43)));
						indiv.setCountry2(formatter.formatCellValue(row.getCell(44)));
						indiv.setProvince2(formatter.formatCellValue(row.getCell(45)));
						indiv.setCity2(formatter.formatCellValue(row.getCell(46)));
						indiv.setBarangay2(formatter.formatCellValue(row.getCell(47)));
						indiv.setPostalcode2(formatter.formatCellValue(row.getCell(48)));
						indiv.setHomeownership2(formatter.formatCellValue(row.getCell(49)));
						indiv.setOccupiedsince2(DateTimeUtil.convertToDate(formatter.formatCellValue(row.getCell(50)),
								DateTimeUtil.DATE_FORMAT_MM_DD_YYYY));
//						indiv.setAddresstype2(formatter.formatCellValue(row.getCell(51)));
						indiv.setCommunitiestype2(formatter.formatCellValue(row.getCell(51)));
						indiv.setFulladdress2(FullDataEntryServiceImpl.fullAddress(indiv.getStreetno2(),
								indiv.getSubdivision2(), indiv.getBarangay2(), indiv.getCity2(), indiv.getProvince2(),
								indiv.getCountry2(), indiv.getPostalcode2()));
						indiv.setSpousetitle(formatter.formatCellValue(row.getCell(52)));
						indiv.setSpousefirstname(formatter.formatCellValue(row.getCell(53)));
						indiv.setSpousemiddlename(formatter.formatCellValue(row.getCell(54)));
						indiv.setSpouselastname(formatter.formatCellValue(row.getCell(55)));
						indiv.setSpousesuffix(formatter.formatCellValue(row.getCell(56)));
						indiv.setFathertitle(formatter.formatCellValue(row.getCell(57)));
						indiv.setFatherfirstname(formatter.formatCellValue(row.getCell(58)));
						indiv.setFathermiddlename(formatter.formatCellValue(row.getCell(59)));
						indiv.setFatherlastname(formatter.formatCellValue(row.getCell(60)));
						indiv.setFathersuffix(formatter.formatCellValue(row.getCell(61)));
						indiv.setMothertitle(formatter.formatCellValue(row.getCell(62)));
						indiv.setMotherfirstname(formatter.formatCellValue(row.getCell(63)));
						indiv.setMothermiddlename(formatter.formatCellValue(row.getCell(64)));
						indiv.setMotherlastname(formatter.formatCellValue(row.getCell(65)));
						indiv.setBeneficiarytitle(formatter.formatCellValue(row.getCell(66)));
						indiv.setBeneficiaryfirstname(formatter.formatCellValue(row.getCell(67)));
						indiv.setBeneficiarymiddlename(formatter.formatCellValue(row.getCell(68)));
						indiv.setBeneficiarylastname(formatter.formatCellValue(row.getCell(69)));
						indiv.setBeneficiarysuffix(formatter.formatCellValue(row.getCell(70)));
						indiv.setBeneficiarydateofbirth(DateTimeUtil.convertToDate(
								formatter.formatCellValue(row.getCell(71)), DateTimeUtil.DATE_FORMAT_MM_DD_YYYY));
						indiv.setRelationtoborrower(formatter.formatCellValue(row.getCell(72)));
						indiv.setCifno(cifNoGenerator.generateCIFNoIndiv());
						indiv.setRemarks(cifmain.getOldcifno());
						indiv.setDatemembership(DateTimeUtil.convertToDate(formatter.formatCellValue(row.getCell(73)),
								DateTimeUtil.DATE_FORMAT_MM_DD_YYYY));
						// indiv.setDatemembership(formatter.formatCellValue(row.getCell(73)));
						indiv.setMembershiptype(formatter.formatCellValue(row.getCell(74)));
						cifmain.setCifno(indiv.getCifno());
						cifmain.setFullname(indiv.getLastname() + (indiv.getSuffix().trim().length() > 0 ? " " + indiv.getSuffix() : "")  + ", " + indiv.getFirstname()
								+ (indiv.getMiddlename().trim().length() > 0 ? " " + indiv.getMiddlename() : ""));
						cifmain.setFulladdress1(indiv.getFulladdress1());
						cifmain.setFulladdress2(indiv.getFulladdress2());

//						param.put("fullname", cifmain.getFullname());
//						Tbcifmain cifdup = (Tbcifmain) dbsrvcCIF
//								.executeUniqueHQLQuery("FROM Tbcifmain WHERE fullname=:fullname ", param);
//						if (cifdup == null) {
						dbsrvcCIF.save(cifmain);
						dbsrvcCIF.save(indiv);
//						}
//						System.out.println(i + " << 	 >> " + cifmain.getOldcifno());
					} else {
						// CORPORATE
//						Tbcifcorporate corp = new Tbcifcorporate();
			...
			if (sheetEmployment != null) {
				...
				log.setErrordescription("");
				log.setEventdate(new Date());
				log.setEventname("BATCH CIF EMPLOYMENT");
				log.setModulename("BATCH UPLOAD");
				log.setNextdate(branch.getNextbusinessdate());
				log.setUniquekey(batchcode);
				dbsrvc.save(log);
				for (int iEmployment = 1; iEmployment < sheetEmployment.getPhysicalNumberOfRows(); iEmployment++) {
					Row row = sheetEmployment.getRow(iEmployment);
					System.out.println(iEmployment + " >> " + formatter.formatCellValue(row.getCell(0)));
					Tbcifemployment employment = new Tbcifemployment();
					param.put("oldcifno", formatter.formatCellValue(row.getCell(0)));
					Tbcifmain main = (Tbcifmain) dbsrvcCIF
							.executeUniqueHQLQuery("FROM Tbcifmain WHERE oldcifno=:oldcifno", param);
					if (main == null) {
						log.setErrordescription("Old CIF number not found.");
						log.setUniquekey(formatter.formatCellValue(row.getCell(1)) + " " + iEmployment);
						dbsrvc.save(log);
						System.out.println("OLD CIF NUMBER NOT FOUND >> " + formatter.formatCellValue(row.getCell(0)));
						continue;
					}
					employment.setCifno(main.getCifno());
					employment.setEmployername(formatter.formatCellValue(row.getCell(1)));
					employment.setEmployeraddress(formatter.formatCellValue(row.getCell(2)));
					employment.setEmployerphoneno(formatter.formatCellValue(row.getCell(3)));
					employment.setCountrycodephone(formatter.formatCellValue(row.getCell(3)));
					employment.setDatehiredfrom(DateTimeUtil.convertToDate(formatter.formatCellValue(row.getCell(4)),
							DateTimeUtil.DATE_FORMAT_MM_DD_YYYY));
//					employment.setDatehiredto(DateTimeUtil.convertToDate(formatter.formatCellValue(row.getCell(5)),
//							DateTimeUtil.DATE_FORMAT_MM_DD_YYYY));
					employment.setIspresent(row.getCell(5).getBooleanCellValue());
					employment.setGrossincome((int) row.getCell(6).getNumericCellValue());
					employment.setPosition(formatter.formatCellValue(row.getCell(8)));
					employment.setPositioncategory(formatter.formatCellValue(row.getCell(9)));
					employment.setEmpstatus(formatter.formatCellValue(row.getCell(10)));
					dbsrvcCIF.save(employment);
				}
			}
			if (sheetBusiness != null) {
				Tblogs log = new Tblogs();
				String batchcode = (String) dbsrvc.executeUniqueSQLQuery(
						"DECLARE @txrefno VARCHAR(20) EXEC SEQGENERATE_PROD 'BTX', @txrefno OUTPUT", param);
				log.setCurrentdate(branch.getCurrentbusinessdate());
				...
				log.setErrordescription("");
				log.setEventdate(new Date());
				log.setEventname("BATCH CIF BUSINESS");
				log.setModulename("BATCH UPLOAD");
				log.setNextdate(branch.getNextbusinessdate());
				log.setUniquekey(batchcode);
				dbsrvc.save(log);
				for (int iBusiness = 1; iBusiness < sheetBusiness.getPhysicalNumberOfRows(); iBusiness++) {
					Row row = sheetBusiness.getRow(iBusiness);
					System.out.println(iBusiness + " >> " + formatter.formatCellValue(row.getCell(0)));
					Tbcifbusiness business = new Tbcifbusiness();
					...
					if (main == null) {
						log.setErrordescription("Old CIF number not found.");
						log.setUniquekey(formatter.formatCellValue(row.getCell(0)) + " " + iBusiness);
						dbsrvc.save(log);
						System.out.println("OLD CIF NUMBER NOT FOUND >> " + formatter.formatCellValue(row.getCell(0)));
						continue;
					}
					business.setCifno(main.getCifno());
					business.setBusinessname(formatter.formatCellValue(row.getCell(1)));
					business.setFulladdress1(formatter.formatCellValue(row.getCell(2)));
					business.setStreetno1(formatter.formatCellValue(row.getCell(2)));
					business.setCountrycodephone(formatter.formatCellValue(row.getCell(3)));
					business.setBusinessphoneno(formatter.formatCellValue(row.getCell(3)));
					business.setCountrycodefax(formatter.formatCellValue(row.getCell(5)));
					business.setEmailaddress(formatter.formatCellValue(row.getCell(6)));
					business.setGrossincome(
							BigDecimal.valueOf(row.getCell(7).getNumericCellValue()).setScale(2, RoundingMode.HALF_UP));
					business.setBusinessclass(formatter.formatCellValue(row.getCell(8)));
					dbsrvcCIF.save(business);
				}
			}
			workbook.close();
//			results.add(new MigrationResultForm("CIF ", filepath));
		} catch (Exception e) {
	...
	public List<MigrationResultForm> uploadDeposit(String filepath) {
		...
		try {
			...
			Workbook workbook = new XSSFWorkbook(file);
			DataFormatter formatter = new DataFormatter();
			Sheet sheetDeposit = null;

			for (Sheet sheet : workbook) {
				if (sheet.getSheetName().equals("DEPOSIT")) {
					sheetDeposit = sheet;
				}
			}
			...
			if (sheetDeposit != null) {
				...
				log.setErrordescription("");
				log.setEventdate(new Date());
				log.setEventname("BATCH DEPOSIT");
				log.setModulename("BATCH UPLOAD");
				log.setNextdate(branch.getNextbusinessdate());
				log.setUniquekey(batchcode);
				dbsrvc.save(log);
				for (int i = 1; i < sheetDeposit.getPhysicalNumberOfRows(); i++) {
					Row row = sheetDeposit.getRow(i);
					System.out.println(i + " >> " + formatter.formatCellValue(row.getCell(1)) + " >> "
							+ formatter.formatCellValue(row.getCell(2)));
					List<Tbdepositcif> cifList = new ArrayList<Tbdepositcif>();
					param.put("cifno", formatter.formatCellValue(row.getCell(1)).trim());
					param.put("oldcifno", formatter.formatCellValue(row.getCell(1)).trim());
					// MAIN CIF ACCOUNT
					if (cifList == null || cifList.size() == 0) {
						cifList = (List<Tbdepositcif>) dbsrvcCIF.execStoredProc(
								"SELECT cifno, fullname as cifname FROM Tbcifmain WHERE cifno=:cifno and oldcifno=:oldcifno",
								param, Tbdepositcif.class, 1, null);
					}
					if (cifList == null || cifList.size() == 0) {
						cifList = (List<Tbdepositcif>) dbsrvcCIF.execStoredProc(
								"SELECT cifno, fullname as cifname FROM Tbcifmain WHERE oldcifno=:oldcifno", param,
								Tbdepositcif.class, 1, null);
					}

					if (cifList == null || cifList.size() == 0) {
						cifList = (List<Tbdepositcif>) dbsrvcCIF.execStoredProc(
								"SELECT cifno, fullname as cifname FROM Tbcifmain WHERE cifno=:cifno", param,
								Tbdepositcif.class, 1, null);
					}

					if (cifList == null || cifList.size() == 0) {
						log.setErrordescription("CIF number not found.");
						log.setUniquekey(formatter.formatCellValue(row.getCell(1)).trim() + " " + i);
						dbsrvc.save(log);
						continue;
					}
					param.put("acctno", formatter.formatCellValue(row.getCell(2)).trim());
					Tbdeposit dep = new Tbdeposit();
					dep = (Tbdeposit) dbsrvc.executeUniqueHQLQuery("FROM Tbdeposit WHERE employeeno=:acctno", param);
					if (dep != null) {
						param.put("ownershiptype", formatter.formatCellValue(row.getCell(3)));
						Tbdepositcif depositcif = new Tbdepositcif();
						depositcif.setAccountno(dep.getAccountNo());
						depositcif.setCifname(cifList.get(0).getCifname());
						depositcif.setCifno(cifList.get(0).getCifno());
						dbsrvc.save(depositcif);
						Tbcodetablecasa ownershiptype = (Tbcodetablecasa) dbsrvc.executeUniqueHQLQuery(
								"FROM Tbcodetablecasa WHERE codename='OWNERSHIPTYPE' and codevalue=:ownershiptype",
								param);
						dep.setAccountName(
								dep.getAccountName() + " " + ownershiptype.getDesc1() + " " + depositcif.getCifname());
					...
					prod = (Tbprodmatrix) dbsrvc.executeUniqueHQLQuery(
							"FROM Tbprodmatrix WHERE prodgroup=:prodcode AND prodcode=:subprodcode", param);
					if (prod == null) {
						log.setErrordescription("Product not found.");
						log.setUniquekey(formatter.formatCellValue(row.getCell(1)) + " " + i);
						dbsrvc.save(log);
						continue;
					}
					dep.setEmployeeNo(formatter.formatCellValue(row.getCell(2)));
					dep.setAccountBalance(BigDecimal.ZERO);
					dep.setProductCode(prod.getProdgroup());
					dep.setSubProductCode(prod.getProdcode());
					dep.setUnit(branch.getBranchcode());
					dep.setAccountnoata("");
					dep.setSlaidNo(formatter.formatCellValue(row.getCell(1)));
					...
					dep.setJointAcctType(formatter.formatCellValue(row.getCell(3)));
					dep.setOwnershipType(0);
					dep.setLessWtaxAmt(BigDecimal.ZERO);
					dep.setCreatedBy(user.getUsername());
					...
					dep.setTdcno("");
					dep.setLasttxcode("");
					dep.setAccountName(cifList.get(0).getCifname());
					dep.setPlaceholdAmt(
							BigDecimal.valueOf(row.getCell(8).getNumericCellValue()).setScale(2, RoundingMode.HALF_UP));
					dep.setFloatAmount(
							BigDecimal.valueOf(row.getCell(9).getNumericCellValue()).setScale(2, RoundingMode.HALF_UP));
					dep.setEarmarkbal(BigDecimal.valueOf(row.getCell(10).getNumericCellValue()).setScale(2,
							RoundingMode.HALF_UP));
					dep.setGarnishedbal(BigDecimal.valueOf(row.getCell(11).getNumericCellValue()).setScale(2,
							RoundingMode.HALF_UP));
					dep.setBookDate(DateTimeUtil.convertToDate(formatter.formatCellValue(row.getCell(12)),
							DateTimeUtil.DATE_FORMAT_MM_DD_YYYY));
					dep.setMaturityDate(DateTimeUtil.convertToDate(formatter.formatCellValue(row.getCell(13)),
							DateTimeUtil.DATE_FORMAT_MM_DD_YYYY));
					dep.setIntRate(BigDecimal.valueOf(row.getCell(14).getNumericCellValue()).setScale(4,
							RoundingMode.HALF_UP));
					dep.setWtaxrate(BigDecimal.valueOf(row.getCell(15).getNumericCellValue()).setScale(4,
							RoundingMode.HALF_UP));
					dep.setPlacementAmt(BigDecimal.valueOf(row.getCell(17).getNumericCellValue()).setScale(4,
							RoundingMode.HALF_UP));
					dep.setTerm((int) row.getCell(18).getNumericCellValue());
					dep.setLessWtaxAmt(BigDecimal.valueOf(row.getCell(20).getNumericCellValue()).setScale(4,
							RoundingMode.HALF_UP));
					dep.setMatAmt(BigDecimal.valueOf(row.getCell(21).getNumericCellValue()).setScale(4,
							RoundingMode.HALF_UP));
					dep.setAccountStatus((int) row.getCell(28).getNumericCellValue());
					dep.setStatusDate(DateTimeUtil.convertToDate(formatter.formatCellValue(row.getCell(29)),
							DateTimeUtil.DATE_FORMAT_MM_DD_YYYY));
					dep.setTdcno(formatter.formatCellValue(row.getCell(32)));
					AccountGenericForm acctResult = new AccountGenericForm();
//					acctResult = createDepAcct(dep, cifList,
//							dep.getEmployeeNo().lastIndexOf("-") > 0
//									? dep.getEmployeeNo().substring(0, dep.getEmployeeNo().lastIndexOf("-"))
//									: dep.getEmployeeNo());
					acctResult = createDepAcct(dep, cifList, null);
					if (acctResult.getResult() == 1) {
						if (formatter.formatCellValue(row.getCell(30)).length() > 0) {
							String pbNo = formatter.formatCellValue(row.getCell(30));
							if (formatter.formatCellValue(row.getCell(30)).equals("99")) {
								pbNo = acctResult.getValue();
							}
							...
							pb.setBrid(branch.getBranchcode());
							pb.setIssuancetype("1");
							pb.setNewpssbksn(pbNo);
							pb.setOldpassbksn("");
							pb.setTxby(user.getUserid());
							pb.setTxdate(branch.getCurrentbusinessdate());
							miscSrvc.passbookIssuance(pb);
						}
						if (dep.getProductCode().equals("40")) {
							param.put("newacctno", acctResult.getValue());
							Tbdepdetail detail = (Tbdepdetail) dbsrvc
									.executeUniqueHQLQuery("FROM Tbdepdetail WHERE accountno=:newacctno", param);
							detail.setIntbaseyear((int) row.getCell(16).getNumericCellValue());
							dbsrvc.saveOrUpdate(detail);
							...
							td.setBranchcode(user.getBranchcode());
							td.setCoopcode(branch.getCoopcode());
							td.setDispositiontype(formatter.formatCellValue(row.getCell(33)));
							td.setDtbook(dep.getBookDate());
							td.setDtmat(dep.getMaturityDate());
							td.setGrossint(BigDecimal.valueOf(row.getCell(19).getNumericCellValue()).setScale(2,
									RoundingMode.HALF_UP));
							td.setIntrate(dep.getIntRate());
							td.setLessdocstamp(BigDecimal.valueOf(row.getCell(22).getNumericCellValue()).setScale(2,
									RoundingMode.HALF_UP));
							...
							td.setWtaxrate(dep.getWtaxrate());
							acctSrvc.saveTDDetails(td);
							if (dep.getTdcno().length() > 0) {

								Tbtdc tdc = new Tbtdc();
								tdc.setAccountno(td.getAccountno());
								...
								tdc.setIssuedby(user.getUsername());
								...
								tdc.setTdcno(dep.getTdcno());
								tdc.setIntrate(dep.getIntRate());
								dbsrvc.save(tdc);
							}
						}
					...
					if (BigDecimal.valueOf(row.getCell(7).getNumericCellValue()).setScale(2, RoundingMode.HALF_UP)
						...
						Tbtransactioncode tx = (Tbtransactioncode) dbsrvc
								.executeUniqueHQLQuery("FROM Tbtransactioncode WHERE txcode ='112013'", param);
						DepositTransactionForm txForm = new DepositTransactionForm();
						txForm.setValuedate(DateTimeUtil.convertToDate(formatter.formatCellValue(row.getCell(27)),
								DateTimeUtil.DATE_FORMAT_MM_DD_YYYY));
						...
						txForm.setTxamount(BigDecimal.valueOf(row.getCell(7).getNumericCellValue()).setScale(2,
								RoundingMode.HALF_UP));
						txForm.setTxcode(tx.getTxcode());
						txForm.setAccountno(acctResult.getValue());
						txForm.setBatchcode(batchcode);
						depSrvc.casaTransaction(txForm, tx, null);
					}
//					// PLACEMENT AMOUNT - CREDIT MEMO
//					if (BigDecimal.valueOf(row.getCell(17).getNumericCellValue()).setScale(2, RoundingMode.HALF_UP)
//							.compareTo(BigDecimal.ZERO) == 1) {
//						Tbtransactioncode tx = (Tbtransactioncode) dbsrvc
//								.executeUniqueHQLQuery("FROM Tbtransactioncode WHERE txcode ='112013'", param);
//						DepositTransactionForm txForm = new DepositTransactionForm();
...
//						txForm.setTxcode(tx.getTxcode());
//						DepositTransactionService depSrvc = new DepositTransactionServiceImpl();
//						txForm.setTxamount(BigDecimal.valueOf(row.getCell(17).getNumericCellValue()).setScale(2,
//								RoundingMode.HALF_UP));
//						txForm.setTxcode(tx.getTxcode());
//						txForm.setAccountno(acctResult.getValue());
//						txForm.setBatchcode(batchcode);
...
//					}
					...
					// ACRRUED INTEREST ON TERM - INTEREST ACCRUAL
					if (BigDecimal.valueOf(row.getCell(23).getNumericCellValue()).setScale(2, RoundingMode.HALF_UP)
							.compareTo(BigDecimal.ZERO) == 1) {
						Tbtransactioncode tx = (Tbtransactioncode) dbsrvc
								.executeUniqueHQLQuery("FROM Tbtransactioncode WHERE txcode ='911400'", param);
						DepositTransactionForm txForm = new DepositTransactionForm();
						txForm.setValuedate(DateTimeUtil.convertToDate(formatter.formatCellValue(row.getCell(36)),
								DateTimeUtil.DATE_FORMAT_MM_DD_YYYY));
						txForm.setErrorcorrect(false);
						txForm.setReason("9");
						txForm.setRemarks("Migration");
						...
						txForm.setTxcode(tx.getTxcode());
						DepositTransactionService depSrvc = new DepositTransactionServiceImpl();
						txForm.setTxamount(BigDecimal.valueOf(row.getCell(23).getNumericCellValue()).setScale(2,
								RoundingMode.HALF_UP));
						txForm.setTxcode(tx.getTxcode());
						txForm.setAccountno(acctResult.getValue());
						txForm.setBatchcode(batchcode);
					...
					// INTEREST CREDIT ON TERM - INTEREST CREDIT
//					if (BigDecimal.valueOf(row.getCell(24).getNumericCellValue()).setScale(2, RoundingMode.HALF_UP)
//							.compareTo(BigDecimal.ZERO) == 1) {
//						Tbtransactioncode tx = (Tbtransactioncode) dbsrvc
//								.executeUniqueHQLQuery("FROM Tbtransactioncode WHERE txcode ='911401'", param);
//						DepositTransactionForm txForm = new DepositTransactionForm();
//						txForm.setValuedate(DateTimeUtil.convertToDate(formatter.formatCellValue(row.getCell(29)),
//								DateTimeUtil.DATE_FORMAT_MM_DD_YYYY));
//						txForm.setErrorcorrect(false);
//						txForm.setReason("9");
//						txForm.setRemarks("Migration");
...
//					}
					...
					// WTAX AMOUNT ON TERM - WTAX
//					if (BigDecimal.valueOf(row.getCell(25).getNumericCellValue()).setScale(2, RoundingMode.HALF_UP)
//							.compareTo(BigDecimal.ZERO) == 1) {
//						Tbtransactioncode tx = (Tbtransactioncode) dbsrvc
//								.executeUniqueHQLQuery("FROM Tbtransactioncode WHERE txcode ='912401'", param);
//						DepositTransactionForm txForm = new DepositTransactionForm();
//						txForm.setValuedate(DateTimeUtil.convertToDate(formatter.formatCellValue(row.getCell(36)),
//								DateTimeUtil.DATE_FORMAT_MM_DD_YYYY));
//						txForm.setErrorcorrect(false);
...
//						txForm.setTxcode(tx.getTxcode());
//						DepositTransactionService depSrvc = new DepositTransactionServiceImpl();
//						txForm.setTxamount(BigDecimal.valueOf(row.getCell(24).getNumericCellValue()).setScale(2,
//								RoundingMode.HALF_UP));
//						txForm.setTxcode(tx.getTxcode());
//						txForm.setAccountno(acctResult.getValue());
//						txForm.setBatchcode(batchcode);
...
//					}
					...
					// ACRRUED ADDITIONAL INTEREST - INTEREST ACCRUAL
//					if (BigDecimal.valueOf(row.getCell(25).getNumericCellValue()).setScale(2, RoundingMode.HALF_UP)
//							.compareTo(BigDecimal.ZERO) == 1) {
//						Tbtransactioncode tx = (Tbtransactioncode) dbsrvc
//								.executeUniqueHQLQuery("FROM Tbtransactioncode WHERE txcode ='911400'", param);
//						DepositTransactionForm txForm = new DepositTransactionForm();
//						txForm.setValuedate(DateTimeUtil.convertToDate(formatter.formatCellValue(row.getCell(34)),
//								DateTimeUtil.DATE_FORMAT_MM_DD_YYYY));
//						txForm.setErrorcorrect(false);
//						txForm.setReason("9");
//						txForm.setRemarks("Migration");
...
//						txForm.setTxcode(tx.getTxcode());
//						DepositTransactionService depSrvc = new DepositTransactionServiceImpl();
//						txForm.setTxamount(BigDecimal.valueOf(row.getCell(25).getNumericCellValue()).setScale(2,
//								RoundingMode.HALF_UP));
//						txForm.setTxcode(tx.getTxcode());
//						txForm.setAccountno(acctResult.getValue());
//						txForm.setBatchcode(batchcode);
...
//					}
					...
					// ADDITIONAL INTEREST CREDIT - INTEREST CREDIT
//					if (BigDecimal.valueOf(row.getCell(25).getNumericCellValue()).setScale(2, RoundingMode.HALF_UP)
//							.compareTo(BigDecimal.ZERO) == 1) {
//						Tbtransactioncode tx = (Tbtransactioncode) dbsrvc
//								.executeUniqueHQLQuery("FROM Tbtransactioncode WHERE txcode ='911401'", param);
...
//					}
					...
					// ADDITIONAL WTAX AMOUNT - WTAX
//					if (BigDecimal.valueOf(row.getCell(26).getNumericCellValue()).setScale(2, RoundingMode.HALF_UP)
//							.compareTo(BigDecimal.ZERO) == 1) {
//						Tbtransactioncode tx = (Tbtransactioncode) dbsrvc
//								.executeUniqueHQLQuery("FROM Tbtransactioncode WHERE txcode ='912401'", param);
//						DepositTransactionForm txForm = new DepositTransactionForm();
...
//						txForm.setTxcode(tx.getTxcode());
//						DepositTransactionService depSrvc = new DepositTransactionServiceImpl();
//						txForm.setTxamount(BigDecimal.valueOf(row.getCell(26).getNumericCellValue()).setScale(2,
//								RoundingMode.HALF_UP));
//						txForm.setTxcode(tx.getTxcode());
//						txForm.setAccountno(acctResult.getValue());
//						txForm.setBatchcode(batchcode);
...
//					}
					...
					// WITHDRAWN INTEREST - DEBIT MEMO
//					if (BigDecimal.valueOf(row.getCell(27).getNumericCellValue()).setScale(2, RoundingMode.HALF_UP)
//							.compareTo(BigDecimal.ZERO) == 1) {
//						Tbtransactioncode tx = (Tbtransactioncode) dbsrvc
...
//						txForm.setTxcode(tx.getTxcode());
//						DepositTransactionService depSrvc = new DepositTransactionServiceImpl();
//						txForm.setTxamount(BigDecimal.valueOf(row.getCell(27).getNumericCellValue()).setScale(2,
//								RoundingMode.HALF_UP));
//						txForm.setTxcode(tx.getTxcode());
//						txForm.setAccountno(acctResult.getValue());
//						txForm.setBatchcode(batchcode);
//						depSrvc.casaTransaction(txForm, tx, null);
//					}
					param.put("acctno", acctResult.getValue());
					dep = (Tbdeposit) dbsrvc.executeUniqueHQLQuery("FROM Tbdeposit WHERE accountno=:acctno", param);
					dep.setLasttxdate(DateTimeUtil.convertToDate(formatter.formatCellValue(row.getCell(27)),
							DateTimeUtil.DATE_FORMAT_MM_DD_YYYY));
					dep.setAccountStatus((int) row.getCell(28).getNumericCellValue());
					dep.setInterestearned(BigDecimal.valueOf(row.getCell(26).getNumericCellValue()).setScale(2,
							RoundingMode.HALF_UP));
					dbsrvc.saveOrUpdate(dep);
				}
			}
	...
	public List<MigrationResultForm> uploadLoans(String filepath) {
		...
		try {
			...
			Workbook workbook = new XSSFWorkbook(file);
			DataFormatter formatter = new DataFormatter();
			Sheet sheetLoans = null;
			for (Sheet sheet : workbook) {
				if (sheet.getSheetName().equals("LOANS")) {
					sheetLoans = sheet;
				}
			}
			if (sheetLoans != null) {
				...
				Tbbranch branch = (Tbbranch) dbService
						.executeUniqueHQLQuery("FROM Tbbranch WHERE branchcode=:branchcode", param);
				Tblogs log = new Tblogs();
				String batchcode = (String) dbService.executeUniqueSQLQuery(
						"DECLARE @txrefno VARCHAR(20) EXEC SEQGENERATE_PROD 'BTX', @txrefno OUTPUT", param);
				log.setCurrentdate(branch.getCurrentbusinessdate());
				log.setDescription(
						"BATCH LOAN UPLOAD - " + filepath.substring(filepath.lastIndexOf("\\") + 1, filepath.length()));
				log.setErrordescription("");
				log.setEventdate(new Date());
				log.setEventname("BATCH LOAN");
				log.setModulename("BATCH UPLOAD");
				log.setNextdate(branch.getNextbusinessdate());
				log.setUniquekey(batchcode);
				dbService.save(log);

				for (int i = 1; i < sheetLoans.getPhysicalNumberOfRows(); i++) {
					Row row = sheetLoans.getRow(i);
					if (formatter.formatCellValue(row.getCell(1)) == null
							|| formatter.formatCellValue(row.getCell(1)).isEmpty()) {
						System.out.println("CIF Number Empty.");
						continue;
					}
					System.out.println(i + " >> " + formatter.formatCellValue(row.getCell(1)));
					param.put("oldcifno", formatter.formatCellValue(row.getCell(1)));
					param.put("cifno", formatter.formatCellValue(row.getCell(2)));
					// MAIN CIF ACCOUNT
					Tbcifmain cifmain = (Tbcifmain) dbsrvcCIF
							.executeUniqueHQLQuery("FROM Tbcifmain WHERE oldcifno=:oldcifno", param);
					if (cifmain == null) {
						cifmain = (Tbcifmain) dbsrvcCIF.executeUniqueHQLQuery("FROM Tbcifmain WHERE cifno=:cifno",
								param);
					}
					if (cifmain == null) {
						System.out.println("CIF NUMBER NOT FOUND >> " + formatter.formatCellValue(row.getCell(1)));
						log.setErrordescription("CIF number not found.");
						log.setUniquekey(formatter.formatCellValue(row.getCell(1)) + " " + i);
						dbService.save(log);
						continue;
					...
					param.put("branchcode", formatter.formatCellValue(row.getCell(0)));
					branch = (Tbbranch) dbService.executeUniqueHQLQuery("FROM Tbbranch WHERE branchcode=:branchcode",
							param);
					Tblstapp lstapp = new Tblstapp();
					lstapp.setCustomertype(cifmain.getCustomertype());
					lstapp.setApplicationtype(3);
					lstapp.setCifno(cifmain.getCifno());
					lstapp.setCifname(cifmain.getFullname());
					lstapp.setIsexisting(true);
					lstapp.setApplicationdate(new Date());
					lstapp.setDatecreated(new Date());
					lstapp.setCreatedby(user.getUsername());
					lstapp.setApplicationstatus(8);
					lstapp.setLoanproduct(formatter.formatCellValue(row.getCell(3)));
					if (lstapp.getLoanproduct() == null || lstapp.getLoanproduct().isEmpty()) {
						continue;
					}
					lstapp.setIsdoneencoding(true);
					lstapp.setBranchcode(branch.getBranchcode());
					lstapp.setCompanycode(branch.getCoopcode());
					lstapp.setAppno(ApplicationNoGenerator.generateApplicationNo("RE"));
					lstapp.setPnno(
							ApplicationNoGenerator.generatePNNo(lstapp.getCompanycode(), lstapp.getLoanproduct()));
					lstapp.setAccountno(lstapp.getPnno());
					if (dbService.save(lstapp)) {
						LoanCalculatorServiceNew loancalc = new LoanCalculatorServiceImplNew();
						LoanCalculatorForm form = new LoanCalculatorForm();
						form.setProdcode(lstapp.getLoanproduct());
						form.setRepaytype(formatter.formatCellValue(row.getCell(5)));
						form.setInttype(formatter.formatCellValue(row.getCell(6)));
						form.setIntpaytype(formatter.formatCellValue(row.getCell(8)));
						form.setBookingdate(DateTimeUtil.convertToDate(formatter.formatCellValue(row.getCell(9)),
								DateTimeUtil.DATE_FORMAT_MM_DD_YYYY));
						form.setMaturitydate(DateTimeUtil.convertToDate(formatter.formatCellValue(row.getCell(10)),
								DateTimeUtil.DATE_FORMAT_MM_DD_YYYY));
						form.setFirstbookingdate(DateTimeUtil.convertToDate(formatter.formatCellValue(row.getCell(11)),
								DateTimeUtil.DATE_FORMAT_MM_DD_YYYY));
						form.setTerm(BigDecimal.valueOf(row.getCell(12).getNumericCellValue()).setScale(2,
								RoundingMode.HALF_UP));
						form.setTermcycle(formatter.formatCellValue(row.getCell(13)));
						form.setCycle(formatter.formatCellValue(row.getCell(13)));
						form.setIntpaycycle(formatter.formatCellValue(row.getCell(14)));
						form.setPrinpaycycle(formatter.formatCellValue(row.getCell(15)));
						form.setPpynum((int) row.getCell(16).getNumericCellValue());
						form.setRate(BigDecimal.valueOf(row.getCell(17).getNumericCellValue()));
						form.setIntcycle(formatter.formatCellValue(row.getCell(18)));
						form.setAmountfinanced(BigDecimal.valueOf(row.getCell(19).getNumericCellValue()).setScale(2,
								RoundingMode.HALF_UP));
						form.setLoanpurpose(formatter.formatCellValue(row.getCell(34)));
						form.setLoanappno(lstapp.getAppno());
						form.setAmortfee(BigDecimal.ZERO);
						form.setTotalcharges(BigDecimal.ZERO);
						form.setTotalfeeschargesbilled(BigDecimal.ZERO);
						form.setPrinbal(form.getAmountfinanced());
						form.setDuedaterule(formatter.formatCellValue(row.getCell(35)));
						form.setAdvanceinterest(BigDecimal.ZERO);
						form.setTotalcharges(BigDecimal.ZERO);
						form.setIntchargeamt(BigDecimal.ZERO);
						form = loancalc.computeLoan(form);
						param.put("repaytype", form.getRepaytype());
						form.setRepaytypedesc((String) dbService.executeUniqueSQLQuery(
								"SELECT repaymentscheme FROM Tbloanrepaymentscheme WHERE schemecode=CAST(:repaytype as int)",
								param));
						param.put("inttype", form.getInttype());
						form.setInttypedesc((String) dbService.executeUniqueSQLQuery(
								"SELECT desc1 FROM Tbcodetable WHERE codename='INTCOMPTYPE' and codevalue=:inttype",
								param));
						param.put("intpaytype", form.getIntpaytype());
						form.setIntpaytypedesc((String) dbService.executeUniqueSQLQuery(
								"SELECT desc1 FROM Tbcodetable WHERE codename='INTERESTPAYOUT' and codevalue=:intpaytype",
								param));
						param.put("termcycle", form.getCycle());
						form.setTermcycdesc((String) dbService.executeUniqueSQLQuery(
								"SELECT remarks FROM Tbcodetable WHERE codename='PAYMENTCYCLE' and codevalue=:termcycle",
								param));
						param.put("intcycle", form.getIntcycle());
						form.setIntcycdesc((String) dbService.executeUniqueSQLQuery(
								"SELECT desc2 FROM Tbcodetable WHERE codename='PAYMENTCYCLE' and codevalue=:intcycle",
								param));
						param.put("intpaycycle", form.getIntpaycycle());
						form.setIntpaycycdesc((String) dbService.executeUniqueSQLQuery(
								"SELECT cycle FROM Tbcycleperloanscheme WHERE schemecode=:repaytype and pi='I' and cyclecode=:intpaycycle",
								param));
						param.put("prinpaycycle", form.getPrinpaycycle());
						form.setPrinpaycycdesc((String) dbService.executeUniqueSQLQuery(
								"SELECT cycle FROM Tbcycleperloanscheme WHERE schemecode=:repaytype and pi='P' and cyclecode=:prinpaycycle",
								param));
						loancalc.saveLoanDetails(form);
						param.put("appno", lstapp.getAppno());
						Tbaccountinfo acct = (Tbaccountinfo) dbService
								.executeUniqueHQLQuery("FROM Tbaccountinfo WHERE applno=:appno", param);
						acct.setPnno(lstapp.getPnno());
						dbService.saveOrUpdate(acct);
						List<LoanAccountForm> loansForm = new ArrayList<LoanAccountForm>();
						LoanAccountForm loanForm = new LoanAccountForm();
						loanForm.setApplno(lstapp.getAppno());
						loanForm.setPnno(lstapp.getPnno());
						loansForm.add(loanForm);
						LMSEODService lmsSrvc = new LMSEODServiceImpl();
						lmsSrvc.loanBooking(loansForm);
						param.put("acctno", lstapp.getAccountno());
						Tbloans loan = (Tbloans) dbService.executeUniqueHQLQuery("FROM Tbloans WHERE accountno=:acctno",
								param);
						loan.setAr1(BigDecimal.valueOf(row.getCell(24).getNumericCellValue()).setScale(4,
								RoundingMode.HALF_UP));
						loan.setLpcrate(BigDecimal.valueOf(row.getCell(30).getNumericCellValue()).setScale(4,
								RoundingMode.HALF_UP));
						loan.setLpcbal(BigDecimal.valueOf(row.getCell(27).getNumericCellValue()).setScale(4,
								RoundingMode.HALF_UP));
						loan.setAcctsts((int) row.getCell(31).getNumericCellValue());
						loan.setAcctstsDate(DateTimeUtil.convertToDate(formatter.formatCellValue(row.getCell(32)),
								DateTimeUtil.DATE_FORMAT_MM_DD_YYYY));
						loan.setLsttxdt(DateTimeUtil.convertToDate(formatter.formatCellValue(row.getCell(33)),
								DateTimeUtil.DATE_FORMAT_MM_DD_YYYY));
						loan.setPnintmethod(formatter.formatCellValue(row.getCell(7)));
						loan.setRefno(formatter.formatCellValue(row.getCell(2)));
						dbService.saveOrUpdate(loan);

						Tbloanfin loanfin = new Tbloanfin();
						param.put("prinbal", BigDecimal.valueOf(row.getCell(20).getNumericCellValue()).setScale(2,
								RoundingMode.HALF_UP));
						loanfin.setTxprin(BigDecimal.valueOf(row.getCell(21).getNumericCellValue()).setScale(2,
								RoundingMode.HALF_UP));
						param.put("acctno", loan.getAccountno());
						loanfin.setTxint(BigDecimal.valueOf(row.getCell(23).getNumericCellValue()).setScale(2,
								RoundingMode.HALF_UP));
//						loanfin.setTxint(dbService.getSQLAmount(
//								"SELECT ISNULL((select sum(ilint) from tbpaymentsched c where c.accountno = :acctno and prinbal >=:prinbal and ilno <> 0),0)",
//								param));
						loanfin.setTxmisc(BigDecimal.valueOf(row.getCell(25).getNumericCellValue()).setScale(4,
								RoundingMode.HALF_UP));
						loanfin.setTxlpc(BigDecimal.valueOf(row.getCell(28).getNumericCellValue()).setScale(4,
								RoundingMode.HALF_UP));
//						loanfin.setTxmisc(BigDecimal.valueOf(row.getCell(24).getNumericCellValue()).setScale(2,
//								RoundingMode.HALF_UP));
//						loanfin.setTxlpc(BigDecimal.valueOf(row.getCell(25).getNumericCellValue()).setScale(2,
//								RoundingMode.HALF_UP));
						loanfin.setTxamount(loanfin.getTxprin().add(loanfin.getTxint()).add(loanfin.getTxlpc())
								.add(loanfin.getTxmisc()));
						loanfin.setTxamtbal(loanfin.getTxamount());
						if (loanfin.getTxamount().compareTo(BigDecimal.ZERO) == 1) {
							loanfin.setAccountno(loan.getAccountno());
							loanfin.setAddtnlint(BigDecimal.ZERO);
							loanfin.setApprovaldate(new Date());
							loanfin.setApprovedby(user.getUsername());
							loanfin.setArtype(null);
							loanfin.setCifno(cifmain.getCifno());
							loanfin.setClientname(cifmain.getFullname());
							loanfin.setCommrate(BigDecimal.ZERO);
							loanfin.setCommamt(BigDecimal.ZERO);
							loanfin.setCreatedby(user.getUsername());
							loanfin.setCreationdate(new Date());
							loanfin.setLegveh(loan.getLegveh());
							loanfin.setPnno(loan.getAccountno());
							loanfin.setSlaidno(loanfin.getCifno());
							loanfin.setTxcode("50"); // credit adjustment
							loanfin.setTxdate(new Date());
							loanfin.setTxmode("4");
							loanfin.setTxoper(2);
							loanfin.setTxor("");
							loanfin.setTxordate(branch.getCurrentbusinessdate());
							loanfin.setTxposteddate(branch.getCurrentbusinessdate());
							loanfin.setTxrefno(HQLUtil.generateTransactionRefNoDate());
							loanfin.setTxsrce(loan.getAccountno());
							loanfin.setTxstatus("9");
							loanfin.setTxstatusdate(new Date());
							loanfin.setTxvaldt(DateTimeUtil.convertToDate(formatter.formatCellValue(row.getCell(33)),
									DateTimeUtil.DATE_FORMAT_MM_DD_YYYY));
							loanfin.setWtaxamt(BigDecimal.ZERO);
							loanfin.setWtaxrate(BigDecimal.ZERO);
							dbService.save(loanfin);
							TransactionService txSrvc = new TransactionServiceImpl();
							txSrvc.postSinglePayment(loanfin.getTxrefno());
						}
						loan = (Tbloans) dbService.executeUniqueHQLQuery("FROM Tbloans WHERE accountno=:acctno", param);
						loan.setLstpydt(DateTimeUtil.convertToDate(formatter.formatCellValue(row.getCell(33)),
								DateTimeUtil.DATE_FORMAT_MM_DD_YYYY));
						dbService.saveOrUpdate(loan);
					}
				}
	...
	public AccountGenericForm createDepAcct(Tbdeposit dep, List<Tbdepositcif> ciflist, String oldcifno) {
		...
		Map<String, Object> param = HQLUtil.getMap();
		try {
			if (oldcifno != null && oldcifno.length() > 0) {
...
//						+ dep.getAccountNo().substring(dep.getAccountNo().length() - 1, dep.getAccountNo().length()));
				dep.setAccountNo(dep.getUnit() + dep.getSubProductCode()
						+ String.format("%08d", Integer.valueOf(oldcifno.replaceAll("[^0-9]", ""))));
				dep.setAccountNo(dep.getAccountNo() + AccountNumberGenerator.getCheckDigit(dep.getAccountNo(), 93812));
			} else {
				dep.setAccountNo(AccountNumberGenerator.genereteAccountNumber(dep.getUnit(), dep.getSubProductCode(),
						dep.getProductCode()));
			}
//			System.out.println(dep.getAccountNo());
